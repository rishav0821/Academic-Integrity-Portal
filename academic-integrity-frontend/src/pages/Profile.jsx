      ...
      <div style={styles.page}>
        ...
        {isStudent && (
          <>
            ...
            <div style={styles.statsGrid}>
              <StatCard
                icon={<FiAward size={26} color="#0f62b2" />}
                bg="#e8f0fc"
                label="Consistency Score"
                value={performanceData ? `${performanceData.consistencyScore}/100` : "—"}
                sub={performanceData?.consistencyScore >= 90 ? "Excellent" : "Good"}
                subColor="#2e7d32"
              />
              <StatCard
                icon={<FiAlertTriangle size={26} color="#d7285c" />}
                bg="#fce8ee"
                label="Academic Warnings"
                value={performanceData ? performanceData.warnings : "—"}
                sub={performanceData?.warnings === 0 ? "All Clear" : "Action needed"}
                subColor={performanceData?.warnings === 0 ? "#2e7d32" : "#c62828"}
              />
              <StatCard
                icon={<FiCalendar size={26} color="#f6a117" />}
                bg="#fff8e1"
                label="Overall Attendance"
                value={attendanceData ? `${attendanceData.overallAttendance}%` : "—"}
              ...
              />
              <StatCard
                icon={<FiBook size={26} color="#7b1fa2" />}
                bg="#f3e5f5"
                label="Subjects Tracked"
                value={attendanceData?.subjects?.length ?? "—"}
                      ...
                      <div style={styles.barWrap}>
                        <div style={{
                          ...
                          background: s.warning
                            ? "linear-gradient(90deg,#d7285c,#f44336)"
                            : "linear-gradient(90deg,#0f62b2,#42a5f5)"
                        }} />
                      </div>
                      <span style={{
                        ...styles.barLabel,
                        color: s.warning ? "#d7285c" : "#2e7d32"
                      }}>
            ...
            <div style={styles.card}>
              ...
              {performanceData?.allFlags?.length > 0 ? (
                ...
                <div style={styles.allClear}>
                  ...
                  <p style={{ margin: "10px 0 4px", fontWeight: 700, color: "#2e7d32", fontSize: "16px" }}>
                    All Clear
                  </p>
                  <p style={{ margin: 0, color: "#555", fontSize: "13px" }}>
                    No integrity flags on your academic record.
                  </p>
                </div>
              )}
            </div>
        ...
        {!isStudent && (
          <>
            <div style={styles.statsGrid}>
              <StatCard
                icon={<FiBarChart2 size={26} color="#0f62b2" />}
                bg="#e8f0fc"
                label="Subjects Monitored"
                value={gradingData?.consistencyData?.length ?? "—"}
              ...
              <StatCard
                ...
                value={gradingData?.alerts?.length ?? "—"}
                sub={gradingData?.alerts?.length === 0 ? "No anomalies" : "Review needed"}
                subColor={gradingData?.alerts?.length === 0 ? "#2e7d32" : "#c62828"}
              />
              <StatCard
                icon={<FiTrendingUp size={26} color="#f6a117" />}
                bg="#fff8e1"
                label="At-Risk Students"
                value={gradingData?.atRiskStudents?.length ?? "—"}
                sub="Low trust score flagged"
                subColor={gradingData?.atRiskStudents?.length > 0 ? "#c62828" : "#2e7d32"}
              />
              <StatCard
                icon={<FiUser size={26} color="#7b1fa2" />}
                bg="#f3e5f5"
                label="Role"
                value={user?.role?.charAt(0).toUpperCase() + user?.role?.slice(1)}
                sub="Portal access level"
                subColor="#555"
              />
            </div>

            {/* Grading Anomaly Alerts */}
            {gradingData?.alerts?.length > 0 && (
              <div style={styles.card}>
                <h2 style={styles.sectionTitle}>
                  <FiAlertTriangle style={{ marginRight: 8, verticalAlign: "middle", color: "#d7285c" }} />
                  Grading Anomaly Alerts
                </h2>
                  ...
                  <tbody>
                    {gradingData.atRiskStudents.map((s, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #f0f0f0" }}>
                        ...
                        <td style={styles.td}>
                          <span style={{
                            background: s.trustScore < 50 ? "#fce8ee" : "#fff8e1",
            ...
            {gradingData?.alerts?.length === 0 && gradingData?.atRiskStudents?.length === 0 && (
              <div style={styles.card}>
                <div style={styles.allClear}>
                  ...
                  <p style={{ margin: 0, color: "#555", fontSize: "13px" }}>
                    No grading anomalies or at-risk students flagged.
                  </p>
                </div>
              </div>
            )}
...
const styles = {
  ...
  hero: {
    ...
    background: "linear-gradient(135deg,#0f62b2 0%,#1e3a5f 100%)",
    borderRadius: "16px", padding: "28px 32px",
    marginBottom: "24px", color: "#fff", position: "relative"
  },
  avatarRing: {
  ...
  avatar: {
    ...
    background: "linear-gradient(135deg,#f6a117,#d7285c)",
  ...
  statusBadge: {
    display: "inline-flex", alignItems: "center",
    padding: "4px 12px", borderRadius: "20px", fontSize: "12px",
    fontWeight: 700, color: "#fff", background: "rgba(46,125,50,0.85)"
  },
  heroEmail: { margin: 0, fontSize: "14px", color: "rgba(255,255,255,0.8)" },
  editBtn: {
  ...
  statCard: {
    ...
    padding: "20px", boxShadow: "0 2px 10px rgba(0,0,0,0.06)",
  ...
  flagRow: {
    ...
    padding: "10px 14px", background: "#fff5f7",
    border: "1px solid #fcd", borderRadius: "8px", marginBottom: "8px"
  },
  allClear: {
    display: "flex", flexDirection: "column",
    alignItems: "center", padding: "32px", color: "#555"
  },

  /* Table */
  th: {
    padding: "10px 14px", fontSize: "12px",
    fontWeight: 700, color: "#888", textAlign: "left",
  ...
  /* Alert row */
  alertRow: {
    padding: "14px", background: "#fff5f7",
    border: "1px solid #fcd", borderRadius: "8px", marginBottom: "10px"
  },
};

...
export const getGroupCheating = async (req, res) => {
  try {
    ...
    // Group students by subject+semester → find identical marks clusters
    const buckets = {};
    records.forEach(rc => {
      const sub = rc.subject?.name || rc.course || "Unknown";
      const sem = rc.semester || 0;
      ...
      const mark = rc.marks;
      if (!buckets[key][mark]) buckets[key][mark] = [];
      const sName = rc.student?.name || rc.name || rc.studentId || "Unknown";
      buckets[key][mark].push(sName);
    });

    const groups = [];
    for (const [key, markMap] of Object.entries(buckets)) {
      const [subject, sem] = key.split("||");
      for (const [mark, students] of Object.entries(markMap)) {
        if (students.length >= 2) {
          const score = Math.min(0.99, 0.5 + students.length * 0.1);
          groups.push({
            ...
            confidence: students.length >= 4 ? "High" : students.length === 3 ? "Medium" : "Low",
            message: `${students.length} students scored exactly ${mark} marks in ${subject} (Sem ${sem}) — possible collusion.`,
          });
        }
      }
    }
